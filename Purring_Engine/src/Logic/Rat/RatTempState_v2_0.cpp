...
namespace PE
{
	void RatTempSTATE_v2_0::StateEnter(EntityID id)
	{
		p_data = GETSCRIPTDATA(RatScript_v2_0, id);
	}


	void RatTempSTATE_v2_0::StateUpdate(EntityID id, float deltaTime)
	{
		// Can conduct state checks and call for state changes here
	}


	void RatTempSTATE_v2_0::StateExit(EntityID)
	{
