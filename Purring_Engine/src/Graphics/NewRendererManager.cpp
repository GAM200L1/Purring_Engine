...
namespace PE
{
    namespace Graphics
    {
        ...
        void NewRendererManager::DrawTextObjects(std::vector<EntityID> const& r_textObjects, glm::mat4 const& r_toNdc)
        {
            ...
            for (const EntityID& id : r_textObjects)
            {
                ...
                for (std::string const& line : lines)
                {
                    ...
                    DrawTextLine(textComponent, line, textBox.position, currentY, hAlignOffset, vAlignOffset);
        ...
        void NewRendererManager::DrawTextLine(TextComponent const& r_textComponent, std::string const& r_line, vec2 position, float currentY, float hAlignOffset, float vAlignOffset)
        {
